...
**为什么不在历史中加可见性标志**：可见性是"路径 + 当前 `.lfvignore`"的函数，不是 file-id 或快照的属性。同一个 file-id 的快照在不同分支上可能位于不同路径，有的可见有的不可见，按 file-id 打标志没有唯一正确的取值；按快照打标志违反 `snapshots.log` 的不可变性；而 `.lfvignore` 可以改回去，任何存储下来的标志要么得跟着改写，要么就会和规则脱节。路径变为不可见后 file-id 所处的状态——`tracked` 行被扫描删除、历史完整、没有活跃路径——本来就存在（已删除的文件、import 后未激活的 file-id），不需要新的存储状态。
