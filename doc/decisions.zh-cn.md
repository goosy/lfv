...
**更短的路径：清除整个 file-id。** 当一个文件的全部历史都不当，或其中没有值得保留的部分时，步骤 1–2 可以换成清除它的 file-id：若它仍活跃，先用 `lfv delete <file-id> --retire` 让它退役，然后 `lfv drop <file-id>`，再执行 `gc --purge`。这一次就移除该文件的所有分支与标签，包括 `branch-drop` 拒绝清除的当前分支，也不需要重建历史。代价是该文件的历史一点都不会留下，因此当必须保留部分历史时，仍应使用逐分支的路径。
...
relink 有意**不用于**合并两个活文件：`dst` 必须没有活跃路径（`D`、已删除或处于待命），`src` 必须是盘上存在的活文件（存储状态 `modified` 或 `unmodified`）。把两个活的 file-id 合成一个，必然有一侧盘上的文件要消失或被覆盖，语义与复杂度完全是另一个量级。
...
## 31. LFV 可见性在使用时检查，从不记入历史

**为什么不在历史中加可见性标志**：可见性是"路径 + 当前 `.lfvignore`"的函数，不是 file-id 或快照的属性。同一个 file-id 的快照在不同分支上可能位于不同路径，有的可见有的不可见，按 file-id 打标志没有唯一正确的取值；按快照打标志违反 `snapshots.log` 的不可变性；而 `.lfvignore` 可以改回去，任何存储下来的标志要么得跟着改写，要么就会和规则脱节。路径变为不可见后 file-id 所处的状态——`tracked` 行被扫描删除、历史完整、没有活跃路径——本来就存在（已删除的文件、import 后处于待命的 file-id），不需要新的存储状态。

**为什么用一条统一的作用域规则，而不是逐命令检查**：不需要持续监控；在命令确定工作区路径的那一刻调用一次匹配器，开销很小，而匹配器扫描时本就已加载。多数命令操作的是活跃 file-id，其路径已由扫描确认，所以检查只对来自参数或历史的路径有意义。在 spec §4 统一写一次（实现上也只做一次，先于其它所有前置条件），可以避免各命令之间出现差异——逐命令的条款很容易漏，`mv` 的目标路径和 tree rewind 的 manifest 就是例子。

**为什么重新变为可见的路径恢复原 file-id**：路径不可见期间，仓库本身没有任何变化——该 file-id 的 HEAD 仍位于这个路径且有内容。一个 file-id 是否在跟踪中由仓库决定，`tracked` 行只是它的缓存，`rebuild-index` 也会把这一行还给该 file-id。若扫描改为分配新 file-id，结果就取决于缓存行是否碰巧被删过，之后的 `rebuild-index` 还会发现两个 file-id 占有同一路径。恢复原 file-id 后，可见性的一去一回与"OS 删除文件、在任何 `lfv snap` 之前又恢复"完全一致：同一个 file-id，没有改动就是 `unmodified`。出于同样的原因，恢复检查先于改名识别：`rebuild-index` 会无条件把该路径分配给这个 file-id。

**为什么 `lfv rewind <TS-ish>` 不带 `--force` 就拒绝**：还原一个树快照，用户预期的是完整重现；静默跳过条目会留下一个与 tree HEAD 不一致的工作树。要求 `--force`，就是让用户确认这次范围变化。目标 Tree Snapshot 不被改写（它是不可变的）；下一次 `lfv snap --tree` 从已跟踪文件构建，因此不含任何不可见路径，正如它从不包含不活跃的文件。

## 32. 原 file-id：统一的跟踪判定规则、由用户解决的冲突与 `lfv drop`

**为什么扫描与 `rebuild-index` 共用一条跟踪判定规则**：spec §3.4 要求索引等于"重建后再做一次全量扫描"的结果。扫描和 `rebuild-index` 各自判断哪个 file-id 活跃在哪里时，二者就出现了偏差：`rebuild-index` 会给路径不可见或处于动态 untracked 的 file-id 生成行，而扫描会为重建时本该交还给已有 file-id 的路径分配新 file-id。由两者共同调用同一条规则，从结构上杜绝这类偏差，而不是依赖人工核对。

**为什么没有快照的 file-id 也算原 file-id**：在第一条快照之前，`meta.initial_path` 是文件位置的唯一记录，`rebuild-index` 本来就用它保住这个 file-id。把它当作最后记录的路径，扫描就与之保持一致。出于同样的原因，取消一个已消失的 `A` 文件的跟踪时会删除其目录，而不是留下空壳：空壳会一直是该路径的候选，悄悄接收下一个在那里新建的文件。

**为什么冲突从不自动解决**：一条路径有多个原 file-id，不会在日常使用中出现，而是来自 import 历史、跨设备合并仓库或崩溃残留。任何自动的平局裁决（HEAD 最新、ULID 最大）都会替用户决定文件接续哪一段历史，而且每次都以同样的方式替用户决定。因此 LFV 不分配任何 file-id，而是询问用户。

**为什么用户的选择记录为其余候选的待命**：当多个候选争夺同一路径时，仓库本身必须记录用户选了哪一个，否则重建索引会让冲突重新出现（spec §3.4）。曾考虑过几种记录方式。只把选择保留到下一次 snap 是不够的：snap 改变的是所选 file-id 的日志，其余候选仍然声称占有该路径。在 `config.yaml` 中按路径记录绑定，需要一套自己的生命周期规则，而且候选在底层仍在争夺。给落选者追加删除快照，会把身份决定写进内容历史，还会声称文件从一个明明有文件的路径上消失了。完整的 `active` 字段会重复可见性、untracked 列表与 HEAD 快照已经能推导出的信息，还必须与它们全部保持同步。`standby` 很窄：它只表达"不参与争夺"，其余一切仍然推导得出。它只由用户的显式选择设置与清除（`lfv track --as` / `--new`、`lfv revive`），因此用户可以随时切换活跃的 file-id；又因为只有非待命候选参与争夺，新的冲突总是交给用户决定。

**为什么选择通过认领而不是删除来完成**：冲突发生时，该路径上存在文件，且它尚未归属任何 file-id，因此 `lfv delete <file-id>` 会被理解为删除这个文件本身。用户做的是认领——"这个文件是 X"——这属于 `lfv track`；让其余候选待命是认领的结果。

**为什么 import 进来的 file-id 初始即为待命**：import 进来的历史还没有被要求出现在工作树中。在有 `standby` 之前，这个意图只体现为"没有索引行"，而 `rebuild-index` 无法重现这一点。

**为什么 relink 不是结束冲突的手段**：冲突也可以通过让不需要的候选退役（`lfv delete --retire`，§33）而永久结束；属于不当内容时则清除它们。把一个候选 relink 到另一个上，会把它整条链追加到另一段历史末尾，对两段又长又独立的历史没有意义，而且这类候选通常共享对象，往往会被单分支对象唯一性检查拒绝。

## 33. `delete` 保留历史，`drop` 放弃历史

LFV 有两种搁置历史的方式，每个命令都恰好属于其中一种：

- **保留历史**：`lfv delete` 以一条删除快照结束文件在盘上的存在，文件之后可以复活；`lfv delete --retire` 结束整个 file-id 的身份——它再也不拥有路径，但其历史保持可达，所有指向它的 snap-id、标签与树快照都保持有效。
- **放弃历史**：`lfv drop`、`lfv branch-drop` 与 `lfv tag-drop` 移除使快照可达的东西，随后 `lfv gc --purge` 将其物理删除。

**为什么退役写作 `delete --retire`**：它和其它所有 `delete` 一样保留历史，因此在 LFV 中 `delete` 一词从不意味着失去历史，只有 `drop` 才是。该参数标明身份本身随之结束，而普通的 delete 不会（被删除的文件可以复活）。

**为什么 relink 最后是让 src 退役而不是清除它**：relink 复制到 dst 上的链有新的 snap-id 和更晚的时间戳，因此 src 的 snap-id、标签，以及 relink 之前创建的树快照，仍然指向 src 自己的快照。清除 src 会在下一次 purge 后使它们全部失效。因此 relink 是一个组合——续接链、把盘上文件移交给 dst、再用 `lfv delete --retire` 所用的同一步骤让 src 退役——而不是一个原子操作。

**为什么 `branch-delete` 与 `tag-delete` 改为 `branch-drop` 与 `tag-drop`**：移除一个分支或标签就是移除一个 gc 根，只经由它可达的快照会在下一次 purge 时被删除（例如 `tree:detour` 标签往往是它所保留的那个 tree HEAD 的唯一根）。这是清除，而不是 LFV 意义上的删除。

**为什么 `lfv drop` 服务于不当内容**：在它出现之前，删除内容意味着重建每条受影响的分支、再删除原分支（§10）。当一个文件的全部历史都不当时，清除 file-id 一步就能做到同样的事；它相当于清除该 file-id 的所有分支与标签，包括 `branch-drop` 拒绝清除的当前分支。
